...
@section('content')
    ...
    <form action="{{ route('users.index') }}" method="get" class="py-5">
        <input type="text" name="search" id="search" placeholder="Pesquisar" class="md:w-1/6 bg-gray-200 appearance-none border-2 border-gray-200 rounded w-full py-2 px-4 text-gray-700 leading-tight focus:outline-none focus:bg-white focus:border-purple-500">
            ...
            @foreach ($users as $user)
                <tr>
                    <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                        ...
                        @else
                            <img src="{{ url("favicon.ico") }}" alt="" class="object-cover w-20">
                        @endif
                        {{ $user->name }}
                    </td>
